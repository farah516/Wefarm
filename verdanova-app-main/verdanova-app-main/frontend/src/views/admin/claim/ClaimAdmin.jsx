...
const ClaimAdmin = () => {
  ...
  const filteredClaims = useMemo(() => {
          const claims = claimStore?.AllClaims || []
          const term = searchTerm.toLowerCase()
          return claims.filter((claim) => {
            const type = claim?.type.toLowerCase() || ""
            const status = claim?.status.toLowerCase() || ""
            const username =claim?.creator?.fullname.toLowerCase() || ""
            const adminname = claim?.admin?.fullname.toLowerCase() || ""
        ...
        }, [claimStore?.AllClaims, searchTerm])
    ...
    const fetchData = useCallback(async () => {
      try {
        setLoading(true)
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/claim/`)
        if (response.status === 200) {
          claimStore.getClaims(response.data.claims)
        }
      } catch (err) {
  ...
  return (
    <Card extra="relative w-full h-full p-3 mt-10 overflow-hidden">
      ...
      <input
          type="text"
          placeholder="Rechercher par nom des utulisateurs,  type ou statu du reclamtion..."
          value={searchTerm}
