...
const Claim = () => {
  ...
  const filteredClaims = useMemo(() => {
        const claims = claimStore?.UserClaims || []
      ...
      }, [claimStore?.UserClaims, searchTerm])
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setLoading(true)
      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/claim/me/${id}`)
      if (response.status === 200) {
        claimStore.getUserClaims(response.data.claims)
      }
    } catch (err) {
  ...
  return (
    <Card extra="relative w-full h-full p-3 mt-10 overflow-hidden">
      ...
      <input
          type="text"
          placeholder="Rechercher par type ou statu du reclamtion..."
          value={searchTerm}
      ...
      <Modal
        ...
        title={"Ajouter une nouveau réclamation"}
      >
        <ClaimForm onClose={handleCloseModal} />
      </Modal>
    </Card>
  )
}
